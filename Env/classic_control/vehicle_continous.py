...
class VehicleEnv(gym.Env):
    def __init__(self):
        ...
        low_action=np.array([self.min_action],dtype=np.float32)
        high_action=np.array([self.max_action],dtype=np.float32)
        if self.s_num==1:
            low = np.array(
        ...
        #self.action_space = spaces.Box(
            ...
            #dtype=np.float32
        #)
        # self.action_discrete=11
        self.action_space = spaces.Box(low_action,high_action,dtype=np.float32)  # as for right now, use discrete actions
        ...
        if self.veh['fcEffType'] == 4:
            ...
            if self.transKwInAch>0:
                if self.soc>0.6:
                    ...
                    # self.mcMechKw4ForcedFc = min(self.transKwInAch,action[0] * 2 * self.curMaxMechMcKwIn - self.curMaxMechMcKwIn)
...
#             mcMechKwOutAch[i] = max(minMcKw2HelpFc[i],mcKwIfFcIsReq[i])
        ...
        if done == True:
            ...
            # print(self.obtain_output()['soc_list'],file=f)
            # f.close()
            ...
            dataframe_veh.to_csv("veh_data_continue.csv",index=False,sep=',')
